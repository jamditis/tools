...
> - Skipped schema validation on JSON files. Cost: 3 hours debugging a typo in "Claude 4.5 Opus" that caused silent failures.
> - Built custom date picker when browser native input type="date" would have worked. 2 days wasted.
> - No error messages when data fails to load—users just see blank screen. Should show retry button.
